...
const isPalindrome = (head) => {
    ...
    const recursivelyCheck = (currentNode) => {
        if (currentNode) {
            ...
            if (currentNode.val !== preCurrentNode.val) {
                return false;
            }
...
### 24. 环形链表入口
```
1. 给你一个链表的头节点 head ，判断链表中是否有环。如果链表中存在环，则返回链表中环的入口节点；否则，返回 null 。
2. 示例：
    1. 输入：head = [3,2,0,-4], pos = 1
    2. 输出：true
```
...
### 27. 删除链表的倒数第 N 个结点
```
1. 给你一个链表的头节点 head 和一个整数 n ，请你删除链表的倒数第 n 个结点，并且返回链表的头节点。
2. 示例：
...
### 34. 二叉树层序遍历
```
1. 给你一个二叉树的根节点 root ，返回其节点按 层序遍历 得到的节点值。（即逐层地，从左到右访问所有节点）。
2. 示例：
    1. 输入：root = [3,9,20,null,null,15,7]
    2. 输出：[[3],[9,20],[15,7]]
```
...
### 35. 将有序数组转换为二叉搜索树
```
1. 给你一个整数数组 nums ，其中元素已经按 升序 排列，请你将其转换为一棵 高度平衡 二叉搜索树。
...
### 37. 二叉树中第 k 小的元素
```
1. 给你一个二叉搜索树的根节点 root ，返回树中第 k 个最小元素（从 1 开始计数）。
2. 示例：
...
### 38. 二叉树的右视图
```
1. 给你一个二叉树的根节点 root ，想象自己站在它的右侧，按照从顶部到底部的顺序，返回从右侧所能看到的节点值。
2. 示例：
    1. 输入：root = [1,2,3,null,5,null,4]
    2. 输出：[1,3,4]
```
```javaScript
// 二叉树的右视图
